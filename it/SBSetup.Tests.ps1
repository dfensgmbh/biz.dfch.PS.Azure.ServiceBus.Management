# Requires -Modules biz.dfch.PS.Azure.ServiceBus.Management
...
Describe -Tags "SBSetup.Tests" "SBSetup.Tests" {
	...
	Context "SBQueue.Tests" {
		...
		It "SBQueue-FarmUpAndRunning" -Test {
			# Arrange
			
			# Act
			$stateSBFarm = Get-SBFarmStatus;
			
			# Assert
			foreach($state in $stateSBFarm){
				$state.Status | Should Be "Running";
			}
		}
		
		It "SBQueue-GetQueues" {
			# Arrange
			...
			# Assert
			$queues.Count | Should Not BeNullOrEmpty;
			$queues.Count | Should Not Be 0;
		}
		
		It "SBQueue-QueueNOTIFY-WFE" -Test {
			# Arrange
			$Queue = "NOTIFY-WFE"
			...
			# Assert	
			Get-SBQueue -Path $Queue | Should Not Be $null; 
		}
		
		It "SBQueue-QueueNOTIFY-ORCH" -Test {
			# Arrange
			$Queue = "NOTIFY-ORCH"
		...
		It "SBQueue-QueueAUDIT-LOG" -Test {
			# Arrange
			$Queue = "AUDIT-LOG"
		...
		It "SBQueue-NewQueueAndGetDetails" {
			# Arrange
			$queueName = "TestQueue"
			
			# Act
			$newQueue = New-SBQueue -Path $queueName
			
			# Assert
			$newQueue | Should Not Be $null;
			
			# Act get details
			$queue = Get-SBQueue -Path $queueName;
			
			# Assert get details
			$queue.Status | Should Be "Active";
			$queue.AvailabilityStatus | Should Be "Available";
			
			# Act delete queue
			Remove-SBQueue -Path $queueName;
			
			try {
				Get-SBQueue -Path $queueName;
				$queueDeleted = $false;
			} catch {
				$queueDeleted = $true;
			}
			
			$queueDeleted | Should Be $true;
		}
		
		It "SBQueue-CreateMessageSender" {
			# Arrange
	...
	Context "SBTopic.Tests" {
		It "SBTopic-CreateGetDetailsAndDeleteTopic" {
			try {
				...
				# Act
				$topic = New-SBTopic -Path $topicName;
				
				#Assert
				$topic.Path | Should Be $topicName;
				
				# Act get details
				$newTopic = Get-SBTopic -Path $topicName;
				
				# Assert
				$newTopic.AvailabilityStatus | Should Be "Available";
				$newTopic.Status | Should Be "Active";
				
			} finally {
				# Cleanup
				Remove-SBTopic -Path $topicName;
				
				# Act
				try {
					$topic = Get-SBTopic -Path $topicName;
					$queueDeleted = $false;
				} catch {
					$queueDeleted = $true;
				}
